...
const ABORTABLE = new Set(['PENDING_APPROVAL', 'APPROVED', 'RELEASED', 'ACKNOWLEDGED']);
...
const rq = [];
for (const [k, s] of nodes) if (TERMINAL.has(s.state)) { canReachTerminal.add(k); rq.push(k); }
while (rq.length) {
  const t = rq.shift();
...
for (const [, s] of nodes) {
  if (!ABORTABLE.has(s.state)) continue;
  if (next(s, 'abort', {}).state !== 'ABORTED') l1Fail.push(s);
}
...
function canReachSet(startKey, targetStates) {
  ...
  while (q.length) {
    const k = q.shift();
    if (targetStates.has(nodes.get(k).state)) return true;
    for (const t of fwd.get(k)) if (!seen.has(t)) { seen.add(t); q.push(t); }
  }
  return false;
}
const s4Fail = [];
for (const [k, s] of nodes) if (s.state === 'ACKNOWLEDGED' && !canReachSet(k, S4_TERMINALS)) s4Fail.push(s);
