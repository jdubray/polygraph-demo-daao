...
const instance = createInstance({ strict: true, hasAsyncActions: false, instanceName: 'daao-buggy' });
  ...
  component: {
    ...
    acceptors: {
      ...
      approve: (model) => (p, { reject }) => {
        ...
        // PLANTED BUG (Act 2): the distinct-approver guard is deleted, so a
        // redelivered approval from the SAME authority counts twice and drives
        // the order to APPROVED with only one real approver. This is the
        // classic two-person-rule defect. The gate must manufacture the
        // counterexample from state exploration.
