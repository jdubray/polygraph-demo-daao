...
const instance = createInstance({ strict: true, hasAsyncActions: false, instanceName: 'daao-widenack' });
  ...
  component: {
    ...
    acceptors: {
      ...
      acknowledge: (model) => (_p, { reject }) => {
        if (!['RELEASED','APPROVED'].includes(model.state)) return reject('acknowledge-only-when-released');
